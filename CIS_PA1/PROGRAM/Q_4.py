...
if __name__ == '__main__':
    ...
    status = 'unknown'    # change to 'unknown' 
    case = 'k'          # change the case 

    calbody = pd.read_csv('./data/pa1-'+status+'-'+case+'-calbody.txt')
    calreadings = pd.read_csv('./data/pa1-'+status+'-'+case+'-calreadings.txt')
    if status == 'debug':
...
# reading ground-truth from debug cases
    ...
    if status == 'debug':
        ...
        print('Error for each frame:         ', errors_frames)
        print('Average error for all frames:  ', np.mean(errors_frames))
