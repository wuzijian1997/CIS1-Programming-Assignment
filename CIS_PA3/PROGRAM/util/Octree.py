...
class BoundingSphere(object):
    ...
    def ComputeClosestPoint(self, point):
        ...
        Lambda, Mu = np.linalg.lstsq(A, B)[0]
...
class BoundingBoxTreeNode(object):
    ...
    def ConstructSubtrees(self, minCount = 1):
    ...
    def FindClosestPoint(self, v, param):#bound, closest):
        ...
        if len(self.Spheres) == 0:
            return
        dist = self.MaxRadius + self.param['bound']
        if (v - self.UB > dist).any() or (self.LB - v > dist).any(): return
        if self.HaveSubtrees:
            for i in range(2):
                for j in range(2):
                    for k in range(2):
                        self.Subtrees[i][j][k][0].FindClosestPoint(v,self.param)
        else:
            for S in self.Spheres:
                self.UpdateClosest(S,v, self.param)
...
if __name__ == '__main__':
    ...
    c = Tree.param['closest']
    print(c)
    print(Tree.param['bound'])
