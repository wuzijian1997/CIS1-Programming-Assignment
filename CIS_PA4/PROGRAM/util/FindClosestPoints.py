...
class BruteForce(ClosestTriangleSearch):
    def __init__(self, bdsp_list, len):
        self.bdsp_list = bdsp_list

    def FindClosestPoint(self, v, param):
        min_dist = 1.0e5
        for bd in self.bdsp_list:
            c = bd.ComputeClosestPoint(v)
            if np.linalg.norm(v - c) < min_dist:
                min_dist = np.linalg.norm(v-c)
                param['closest'] = c

class SimpleBoundingSphere():
    def __init__(self, bdsp_list, len):
        self.bdsp_list = bdsp_list
    
    def FindClosestPoint(self, v, param):
        for bd in self.bdsp_list:
            if np.linalg.norm(bd.center - v) - bd.radius <= param['bound']:
                h = bd.ComputeClosestPoint(v)
                if np.linalg.norm(h - v) < param['bound']:
                    param['bound'] = np.linalg.norm(h - v)
                    param['closest'] = h
